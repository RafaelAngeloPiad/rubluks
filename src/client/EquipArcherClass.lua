...
local ShowButtonPrompt = ReplicatedStorage:WaitForChild("ShowButtonPromptArcher")
local EquipArcherClass = ReplicatedStorage:WaitForChild("EquipArcherClass")
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer

-- Utility function to check if Archer tool is present in Backpack or Character
local function hasArcherTool()
    local backpack = LocalPlayer:FindFirstChild("Backpack")
    if backpack then
        for _, tool in backpack:GetChildren() do
            if tool:IsA("Tool") and string.find(tool.Name, "Archer") then
                return true
            end
        end
    end
...
local archerToolEquipped = false

-- Function to update the flag when tool is added/removed
local function updateArcherToolFlag()
    archerToolEquipped = hasArcherTool()
end
...
local function setupToolListeners()
    local backpack = LocalPlayer:FindFirstChild("Backpack")
    if backpack then
        backpack.ChildAdded:Connect(updateArcherToolFlag)
        backpack.ChildRemoved:Connect(updateArcherToolFlag)
    end
    LocalPlayer.CharacterAdded:Connect(function(character)
        character.ChildAdded:Connect(updateArcherToolFlag)
        character.ChildRemoved:Connect(updateArcherToolFlag)
        -- Initial check when character spawns
        updateArcherToolFlag()
    end)
    -- Initial check for current character
    if LocalPlayer.Character then
        LocalPlayer.Character.ChildAdded:Connect(updateArcherToolFlag)
        LocalPlayer.Character.ChildRemoved:Connect(updateArcherToolFlag)
    end
end
...
ShowButtonPrompt.OnClientEvent:Connect(function()
    ...
    textLabel.Text = "Do you want to become an Archer?"
...
local function monitorToolRemoval()
    local function checkToolRemoved()
        if not hasArcherTool() then
            archerToolEquipped = false
        end
    end
    local backpack = LocalPlayer:FindFirstChild("Backpack")
    if backpack then
