...
local clearedByPlayerLeaving = false
	...
	while true do
		...
		if not playerNearby then
			if #activeMobs > 0 then
				despawnAll()
				-- Mobs were cleared because the player left the DetectionRadius.
				-- Next time they come back, respawn should be immediate (no RespawnDelay).
				clearedByPlayerLeaving = true
			end
		else
			if #activeMobs == 0 then
				-- If mobs were only despawned because the player left, ignore RespawnDelay.
				if clearedByPlayerLeaving then
					spawnGroup()
					lastSpawnTime = os.clock()
					clearedByPlayerLeaving = false
				-- Otherwise (they were actually killed), enforce RespawnDelay.
				elseif (os.clock() - lastSpawnTime) >= CONFIG.RespawnDelay then
					spawnGroup()
					lastSpawnTime = os.clock()
				end
			end
